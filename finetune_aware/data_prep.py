...
def process_and_split_data(embed, disease_positive_proteins, disease_negative_proteins, subtype_protein_dict, celltype_subtype, subtype_dict, disease, data_split_path, random_state, test_size):
    ...
    if os.path.exists(data_split_path): # Generate new splits
        ...
        y_test = {celltype: np.concatenate([[1 for ind in pos_test_indices if pos_prots_strata[ind] == celltype], [0 for ind in neg_test_indices if neg_prots_strata[ind] == celltype]]) for celltype in celltype_subtype if celltype != "global"}
        ...
        def get_splits(cv):
            ...
            for i, (pos_train_indices, pos_test_indices) in enumerate(cv.split(X=np.arange(len(pos_embed)), groups=pos_prots_group, y=pos_prots_strata)):
                y_test = {celltype: [1 for ind in pos_test_indices if pos_prots_strata[ind] == celltype] for celltype in celltype_subtype if celltype != "global"}
...
def get_labels_from_evidence(celltype_protein_dict: Dict[str, List[str]], therapeutic_areas: list, evidence_dir: str, all_drug_targets_path: str, curated_disease_dir: str, chembl2db_path: str, 
    ...
    for disease in therapeutic_areas:
        ...
        positive_proteins[disease] = {ct: list(disease_drug_targets.intersection(ppi_proteins)) for ct, ppi_proteins in celltype_protein_dict.items() if ct != 'global'}  # PPI proteins associated with the disease with drug or clinical candidate > II's evidence
        negative_proteins[disease] = {ct: list(set(ppi_proteins).difference(all_associated_targets).intersection(druggable_targets)) for ct, ppi_proteins in celltype_protein_dict.items() if ct != 'global'}  # PPI proteins that are not associated with the disease except for text mining, but are still druggable
    ...
    positive_protein_counts_celltype = pd.DataFrame(positive_proteins).rename(index={ind:ind[:-2] for ind in positive_proteins[disease].keys() if ind!='global'}).reset_index().melt(id_vars = ['index']).groupby(by=['index', 'variable']).aggregate(list).applymap(lambda x: len(np.unique(sum(x, start = [])))).reset_index()
